...
export default async function ProcessesPage() {
  ...
  if (showOwnOnly && broker) {
    ...
    processes = ((data ?? []) as unknown as RawProc[]).map((p) => ({
      id: p.id, p1_name: p.clients?.p1_name ?? '', p2_name: p.clients?.p2_name ?? null,
      process_step: p.process_step, updated_at: p.updated_at, broker_id: p.broker_id, brokers: p.brokers ?? null,
    }));
  }
  ...
  if (processes.length > 0) {
    ...
    const { data: docData } = await serviceClient
      .from('document_requests').select('process_id, is_mandatory, status').in('process_id', processIds);
